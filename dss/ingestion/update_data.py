...
class DataUpdater:
    """Orchestrates data ingestion with incremental updates"""
    ...
    async def update_symbol(self, symbol: str, force_full: bool = False) -> bool:
        """Update data for a single symbol incrementally or force full download"""
        try:
            ...
            if force_full:
                # Force full download: get historical data based on config
                ...
                # Get last timestamp from database for incremental update
                ...
                if last_timestamp:
                    ...
                    # First time: get historical data based on config
                    historical_years = config.get("data_provider.historical_years", 5)
                    historical_days = historical_years * 365
                    start_date = end_date - timedelta(days=historical_days)
    ...
    async def update_all(self, symbols: List[str] = None, force_full: bool = False):
        ...
        if plan == "free":
            ...
            # Starter: small batches with concurrency
            batch_size = 10
            concurrent = True
        elif plan in ["developer", "advanced"]:
            # Higher tiers: larger batches
            batch_size = 50
            concurrent = True
        else:
            batch_size = 1
